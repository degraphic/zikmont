...
class FormatoOrdenCompra extends FormatoGeneral {
    ...
    public function Generar($em, $arMovimiento) {
        ob_end_clean();
        $this->Pdf = new FormatoOrdenCompra('P', 'mm', 'letter', $em);
    ...
    public function Header() {
        ...
        $this->Rect(4, 25, 207, 22);
        $this->ln(1);
        $this->SetY(26);
        $this->SetFont('Arial', 'B', 7);
        $List1 = array('PROVEEDOR: ', 'CONTACTO: ', 'DIRECCION:', 'TELEFONOS:', 'E-MAIL:');
        foreach ($List1 as $col) {
            $this->Cell(40, 5, $col, 0, 0, 'L');
            $this->Ln(4);
        }
        // self::$arMovimiento->getTerceroRel()->getCodigoTerceroPk()
        $Datos = array(self::$arMovimiento->getTerceroRel()->getNombreCortoTercero(), '',self::$arMovimiento->getTerceroRel()->getDireccion(), self::$arMovimiento->getTerceroRel()->getTelefono() . " - FAX: " . self::$arMovimiento->getTerceroRel()->getFax());
        ...
        $List1 = array('FECHA: ');
        foreach ($List1 as $col) {
            $this->SetXY(180,$this->getY()-14);
            $this->Cell(40, 5, $col, 0, 0, 'L');
            $this->Ln(3.5);
        }
        
//        $Datos = array(date('Y-m-d'),tself::$arMovimiento->getFecha());
//        $this->SetFont('Arial', '', 8);
//        $this->SetY(26);
//
//        foreach ($Datos as $col) {
...
//        }
    ...
    public function Body() {
        ...
        $totales2 = array(number_format(self::$arMovimiento->getSubTotal(), 2, '.', ','),
            ...
            number_format(self::$arMovimiento->getTotal(), 2, '.', ','));

        $this->SetFont('Arial', '', 7.5);
        $this->SetXY(193, $this->GetY() - 28);
        $this->ln(12);
        ...
        $this->Cell(0, 0, $this->line(6, $this->GetY($this->SetY(130)), 70, $this->GetY($this->SetY(130))) . $this->Text(10, $this->GetY($this->SetY(133)), "SOLICITADO"));
        $this->Cell(0, 0, $this->line(80, $this->GetY($this->SetY(130)), 140, $this->GetY($this->SetY(130))) . $this->Text(90, $this->GetY($this->SetY(133)), "AUTORIZADO"));
        $this->Cell(0, 0, $this->line(150, $this->GetY($this->SetY(130)), 210, $this->GetY($this->SetY(130))) . $this->Text(150, $this->GetY($this->SetY(133)), "FIRMADO RECIBIDO"));
